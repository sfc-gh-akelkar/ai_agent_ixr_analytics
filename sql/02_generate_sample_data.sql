...
SELECT
    ...
    CASE 
        WHEN g.SEQ <= 32 THEN 4500 + g.SEQ
        WHEN g.SEQ = 33 THEN 4532  -- Our "sick" device
        WHEN g.SEQ <= 66 THEN 4500 + g.SEQ
        WHEN g.SEQ = 67 THEN 7821  -- Second problem device
        ELSE 4500 + g.SEQ
    END AS DEVICE_ID,
    ...
    CASE 
        WHEN g.SEQ = 33 THEN DATEADD('day', -1065, CURRENT_DATE())  -- 4532: ~2.9 years old
        WHEN g.SEQ = 67 THEN DATEADD('day', -950, CURRENT_DATE())   -- 7821: ~2.6 years old
        ELSE DATEADD('day', -(UNIFORM(365, 1460, RANDOM())), CURRENT_DATE())
    END AS INSTALLATION_DATE,
    
    'LOC-' || LPAD(g.SEQ, 4, '0') AS LOCATION_ID,
    ...
    -- Environment types
    CASE 
        WHEN g.SEQ = 33 THEN 'Lobby'  -- 4532: High traffic area
        WHEN g.SEQ = 67 THEN 'Waiting Room'
        WHEN (g.SEQ % 4) = 0 THEN 'Lobby'
        WHEN (g.SEQ % 4) = 1 THEN 'Waiting Room'
        WHEN (g.SEQ % 4) = 2 THEN 'Exam Room'
        ELSE 'Hallway'
    END AS ENVIRONMENT_TYPE,
    
    'HW-v' || (2 + (g.SEQ % 3)) || '.0' AS HARDWARE_VERSION,
    
    -- Firmware versions (older versions more problematic)
    CASE 
        WHEN g.SEQ = 33 THEN 'v2.3.8'  -- 4532: Older firmware with known power issues
        WHEN g.SEQ < 20 THEN 'v2.4.1'  -- Newer firmware
        WHEN g.SEQ < 60 THEN 'v2.3.8'  -- Older firmware
        ELSE 'v2.4.0'
    END AS FIRMWARE_VERSION,
    
    CASE 
        WHEN g.SEQ = 33 THEN 'Expired'  -- 4532: Out of warranty
        WHEN g.SEQ = 67 THEN 'Expired'
    ...
    CASE
        WHEN g.SEQ = 33 THEN DATEADD('day', -118, CURRENT_DATE())  -- 4532: Recent maintenance but still degrading
...
SELECT
    'M-2025-' || LPAD(g.SEQ, 5, '0') AS MAINTENANCE_ID,
    ...
    CASE 
        WHEN (g.SEQ % 10) IN (0, 1) THEN  -- Power supply issues
            CASE WHEN UNIFORM(0, 100, RANDOM()) < 68 THEN 'Remote Fix' ELSE 'Part Replacement' END
        WHEN (g.SEQ % 10) = 2 THEN 'Part Replacement'  -- Display always needs replacement
        WHEN (g.SEQ % 10) IN (3, 4) THEN  -- Software issues
            CASE WHEN UNIFORM(0, 100, RANDOM()) < 94 THEN 'Remote Fix' ELSE 'Field Service' END
        WHEN (g.SEQ % 10) = 5 THEN  -- Network
    ...
    CASE 
        WHEN (g.SEQ % 10) IN (0, 1) AND UNIFORM(0, 100, RANDOM()) < 68 THEN UNIFORM(0.15, 0.5, RANDOM())  -- Remote: 9-30 min
        WHEN (g.SEQ % 10) = 2 THEN UNIFORM(24, 72, RANDOM())  -- Hardware replacement
        WHEN (g.SEQ % 10) IN (3, 4, 5) THEN UNIFORM(0.1, 1, RANDOM())  -- Software/network remote
    ...
    CASE 
        WHEN (g.SEQ % 10) IN (0, 1) AND UNIFORM(0, 100, RANDOM()) < 68 THEN TRUE
        WHEN (g.SEQ % 10) = 2 THEN FALSE
        WHEN (g.SEQ % 10) IN (3, 4) AND UNIFORM(0, 100, RANDOM()) < 94 THEN TRUE
        WHEN (g.SEQ % 10) = 5 AND UNIFORM(0, 100, RANDOM()) < 81 THEN TRUE
        ELSE UNIFORM(0, 100, RANDOM()) < 50
    END AS REMOTE_FIX_SUCCESSFUL,
    
    -- Parts replaced
    CASE 
        WHEN (g.SEQ % 10) = 1 THEN 'Power Supply Unit (PSU-500W)'
        WHEN (g.SEQ % 10) = 2 THEN 'Display Panel'
        WHEN (g.SEQ % 10) = 8 THEN 'Cooling Fan'
        ELSE NULL
    END AS PARTS_REPLACED,
    
    -- Costs
    CASE WHEN (g.SEQ % 10) IN (0, 3, 4, 5, 6, 7) THEN 0 ELSE UNIFORM(100, 150, RANDOM()) END AS LABOR_COST_USD,
    CASE 
        WHEN (g.SEQ % 10) = 1 THEN 280
        WHEN (g.SEQ % 10) = 2 THEN 450
        WHEN (g.SEQ % 10) = 8 THEN 60
        ELSE 0
    END AS PARTS_COST_USD,
    CASE WHEN (g.SEQ % 10) IN (0, 3, 4, 5, 6, 7) THEN 0 ELSE UNIFORM(50, 150, RANDOM()) END AS TRAVEL_COST_USD,
    
    CASE 
        WHEN (g.SEQ % 10) IN (0, 3, 4, 5, 6, 7) THEN 0  -- Remote fixes
        WHEN (g.SEQ % 10) = 1 THEN 280 + 120 + 100  -- PSU replacement
        WHEN (g.SEQ % 10) = 2 THEN 450 + 140 + 110  -- Display replacement
        WHEN (g.SEQ % 10) = 8 THEN 60 + 110 + 80
        ELSE UNIFORM(200, 500, RANDOM())
    END AS TOTAL_COST_USD,
    
    -- Downtime
    CASE 
        WHEN (g.SEQ % 10) IN (0, 3, 4, 5, 6, 7) THEN UNIFORM(0.15, 0.5, RANDOM())  -- Remote: minimal
        ELSE UNIFORM(2, 8, RANDOM())  -- Field service: hours
    END AS DOWNTIME_HOURS,
    
    -- Revenue impact ($97/hour average)
    CASE 
        WHEN (g.SEQ % 10) IN (0, 3, 4, 5, 6, 7) THEN UNIFORM(0.15, 0.5, RANDOM()) * 97
    ...
    -- Preventable
    CASE WHEN (g.SEQ % 10) IN (0, 6, 7, 8) THEN TRUE ELSE FALSE END AS PREVENTABLE
